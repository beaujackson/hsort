...
public class VisitorCountrySummarizer {
    private static int LEN = 16;

    private static HashMap<Integer, VisitorCountrySummarizer> summarizers = new HashMap<>();
    ...
    private VisitorCountrySummarizer() {
    }

    public static VisitorCountrySummarizer getInstance(Integer key) {
        if (!summarizers.containsKey(key)) {
            summarizers.put(key, new VisitorCountrySummarizer());
        }
    ...
    public static void printSummary(double threshold) {
        ...
        for (Integer key : summarizers.keySet()) {
            VisitorCountrySummarizer summarizer = getInstance(key);
    ...
    private void printSummary(Integer destinationId) {
        ...
        addSummary(columns, values, "Country ID", destinationId);
